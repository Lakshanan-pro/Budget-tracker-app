...
def transactions():
    ...
    if request.method == 'POST':
        data = request.get_json()
        cur.execute('''INSERT INTO transactions (user_id, type, amount, category, description, date)
                       VALUES (?, ?, ?, ?, ?, ?)''',
        ...
        data = request.get_json()
        cur.execute('DELETE FROM transactions WHERE id = ? AND user_id = ?', (data['id'], user_id))
        conn.commit()
        cur.close()
        conn.close()
